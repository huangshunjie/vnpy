...
_TBL = (
    ...
    "border-bottom:1px solid #45475a;padding:2px 4px;font-size:10px;}"
)
...
class SystemConsoleWindow(QtWidgets.QMainWindow):
    ...
    def _build_kpi_bar(self) -> QtWidgets.QWidget:
        w = QtWidgets.QWidget(); w.setFixedHeight(100)
        w.setStyleSheet(
        ...
        for key, txt, color in [
            ...
            lk = _lbl(txt, f"color:{_MUT};font-size:11px;border:none;background:transparent;")
            lk.setAlignment(QtCore.Qt.AlignmentFlag.AlignCenter)
            lv = _lbl("--", f"color:{color};font-size:20px;font-weight:bold;"
        ...
        h.addLayout(btn_col)
        self._ts_lbl = _lbl("--", f"color:{_MUT};font-size:10px;"
                             f"border:none;background:transparent;")
    ...
    def _build_cards_area(self) -> QtWidgets.QWidget:
        ...
        for lid in sorted(layers.keys()):
            ...
            # layer header bar
            sep = QtWidgets.QWidget(); sep.setFixedHeight(32)
            sep.setStyleSheet(
            ...
            sh.addWidget(_lbl(
                ...
                f"color:{lc};font-size:13px;font-weight:bold;border:none;"))
            sh.addStretch()
    ...
    def _build_bottom_panel(self) -> QtWidgets.QWidget:
        w = QtWidgets.QWidget(); w.setFixedHeight(240)
    ...
    def _build_event_stream(self) -> QtWidgets.QWidget:
        ...
        hdr.addWidget(_lbl("Module State Changes",
                           f"color:{_CYN};font-size:13px;font-weight:bold;border:none;"))
        hdr.addStretch()
        hdr.addWidget(_btn("Clear", _MUT, self._on_clear_events, h=26, fs=11))
        vb.addLayout(hdr)
    ...
    def _build_log_panel(self) -> QtWidgets.QWidget:
        ...
        hdr.addWidget(_lbl("Console Log",
                           f"color:{_YLW};font-size:13px;font-weight:bold;border:none;"))
        hdr.addStretch()
        hdr.addWidget(_btn("Clear", _MUT, self._on_clear_log, h=26, fs=11))
        vb.addLayout(hdr)
        self._log_txt = QtWidgets.QPlainTextEdit()
        self._log_txt.setReadOnly(True)
        self._log_txt.setMaximumBlockCount(800)
        self._log_txt.setFont(QtGui.QFont("Consolas", 11))
    ...
    def _update_kpi(self, d: dict) -> None:
        ...
        self._kpi["health"].setStyleSheet(
            f"color:{hs_c};font-size:20px;font-weight:bold;"
            f"border:none;background:transparent;")
        ...
        self._kpi["status"].setStyleSheet(
            f"color:{sc};font-size:20px;font-weight:bold;"
            f"border:none;background:transparent;")
        ...
        if err_n > 0:
            self._kpi["errors"].setStyleSheet(
                f"color:{_RED};font-size:20px;font-weight:bold;"
                f"border:none;background:transparent;")
