"""write_ws_p3.py — ProjectExplorer"""
import pathlib, ast
...
class ProjectExplorer(QWidget):
    ...
    def _init_ui(self):
        ...
        self._tree.itemDoubleClicked.connect(self._on_dbl_click)
    ...
    def _add_starred_root(self):
        ...
        root_item = QTreeWidgetItem(["\u2b50  \u6536\u85cf\u5939"])
        root_item.setData(0, ROLE_TYPE, NODE_STARRED)
        root_item.setData(0, ROLE_ID,   "starred")
        font = QFont()
        font.setBold(True)
        root_item.setFont(0, font)
        for proj in starred:
            root_item.addChild(self._make_project_item(proj))
    ...
    def _add_workspace_node(self, workspace_id: str):
        ...
        if not ws:
            return
        ws_item = QTreeWidgetItem(["\U0001f5c2  " + ws.name])
    ...
    def _add_folder_children(self, parent: QTreeWidgetItem, proj: ProjectRecord):
    ...
    def _add_sub_folders(self, parent, parent_id, all_folders):
    ...
    def _on_dbl_click(self, item: QTreeWidgetItem, col: int):
    ...
    def _on_context_menu(self, pos):
        ...
        if ntype == NODE_PROJECT:
            ...
            menu.addSeparator()
            a_star = menu.addAction(
                "\u2606  \u53d6\u6d88\u6536\u85cf" if proj.starred
                else "\u2b50  \u52a0\u5165\u6536\u85cf")
            menu.addSeparator()
            sm = menu.addMenu("\u📌  \u8bbe\u7f6e\u72b6\u6001")
            ...
            if action == a_edit:
                ...
                if proj.starred:
                    self._engine.unstar_project(nid)
                else:
                    self._engine.star_project(nid)
            ...
            a_switch  = menu.addAction("\U0001f504  \u5207\u6362\u5230\u6b64\u5de5\u4f5c\u533a")
            a_archive = menu.addAction("\U0001f4e6  \u5f52\u6863\u5de5\u4f5c\u533a")
            action = menu.exec(self._tree.viewport().mapToGlobal(pos))
            if action == a_switch:
    ...
    def select_project(self, project_id: str):
        def _find(parent):
            for i in range(parent.childCount()):
                ...
                if (child.data(0, ROLE_TYPE) == NODE_PROJECT
                        and child.data(0, ROLE_ID) == project_id):
...
with open(P, "a", encoding="utf-8") as f:
    f.write(PART3)
print("PART3 written OK, total size:", P.stat().st_size)
