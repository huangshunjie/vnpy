...
Phase 4: Universality Scoring Engine â€” å››ç»´æ™®é€‚æ€§è¯„åˆ†å¼•æ“Žã€‚
Score âˆˆ [0,1]ï¼ŒGrade: UNIVERSAL / PORTABLE / LOCAL / FRAGILE
å¤ç”¨ Phase 2/3 ç¼“å­˜ï¼šstructure_cache / transfer_cache / alignment_cache
"""
...
class UniversalityEngine:
    """æ™®é€‚æ€§è¯„åˆ†å¼•æ“Žï¼ˆPhase 4 å®Œæ•´å®žçŽ°ï¼‰ã€‚"""
    ...
    def inject_caches(self, structure_cache: dict,
        ...
        """æ³¨å…¥ Phase 2/3 ç¼“å­˜ï¼Œå®žçŽ°è·¨é˜¶æ®µæˆæžœå¤ç”¨ã€‚"""
        self._structure_cache = structure_cache
        self._transfer_cache  = transfer_cache
        self._alignment_cache = alignment_cache

    # â”€â”€ æ ¸å¿ƒï¼šscore() â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
    ...
    def score(self, alpha_id: str, markets: list[str] | None = None,
        ...
        record = UniversalityScoreRecord(
            ...
            dim_cross_market       = _dim(
                ...
                f"{n_transferable}/{len(target)} å¸‚åœºå¯è¿ç§»ï¼Œå¹³å‡T={avg_t:.3f}"),
            dim_regime             = _dim(
                "regime_robustness", rr, "regime_robustness",
                f"å¹³å‡å¯¹é½åˆ†={avg_align:.3f}ï¼Œå¹³å‡Regimeä¸å˜æ€§={sum(reg_invs)/max(len(reg_invs),1):.3f}"),
            dim_structural         = _dim(
                "structural_invariance", si, "structural_invariance",
                f"å¹³å‡ICè¡°å‡={avg_decay:.3f}ï¼Œå¯è¿ç§»æ€§å…ˆéªŒå‡å€¼={sum(port_scores)/max(len(port_scores),1):.3f}"),
            dim_execution          = _dim(
                "execution_independence", ei, "execution_independence",
                f"vol_sens={vol_sens:.2f}ï¼Œliq_sens={liq_sens:.2f}ï¼Œvol_cv={_cv(vol_scales):.3f}"),
