...
from vnpy.kline_behavior_lab.ui.pattern_stats_tab import PatternStatsTab
...
# 从共享成分股模块动态加载，如果没有缓存则使用少量备用代码

def _load_index_pool(index_code: str) -> List[str]:
...
class BehaviorResearchTab(QWidget):
    ...
    def _build_mid(self):
        ...
        self._pattern_stats_tab = PatternStatsTab(self)

        self._tab.addTab(self._pattern_stats_tab, "📊 形态统计")
    ...
    def _build_right(self):
        ...
        v.addWidget(_lbl("市场/板块", _TEAL, 11, True))
        ...
        for label, key in [("全市场", "ALL"), ("沪市", "SSE"), ("深市", "SZSE"), ("北交所", "BSE")]:
            ...
            b.setStyleSheet(_small_btn_ss)

            b.clicked.connect(lambda checked, k=key: self._set_exchange_pool(k))

            exch_row.addWidget(b)

        v.addLayout(exch_row)
        ...
        v.addLayout(board_row)



        # ━━ 指数成分 ━━

        v.addWidget(_lbl("指数成分", _TEAL, 11, True))
        ...
        for idx, (label, pool) in enumerate(_QUICK_POOL_ITEMS):

            b = QPushButton(label)

            b.setStyleSheet(_idx_btn_ss)
        ...
        more_row.addWidget(_lbl("更多指数:", _MUT, 11))

        self._index_combo = QComboBox()

        self._index_combo.setStyleSheet(_COMBO_SS)

        self._index_combo.setFixedHeight(26)

        self._index_combo.addItem("-- 选择指数 --", "")
        ...
        try:
            ...
            _added_codes = {p.replace("IDX:", "") for _, p in _QUICK_POOL_ITEMS if isinstance(p, str) and p.startswith("IDX:")}
            ...
            for cat in _cat_order:
                ...
                for code, info in _ALL_IDX.items():
                    ...
                    if code in _added_codes:

                        continue

                    if not has_items:

                        self._index_combo.addItem(f"━━ {cat} ━━", "")

                        has_items = True

                    self._index_combo.addItem(f"  {info['name']} ({code})", f"IDX:{code}")

        except Exception:

            pass

        self._index_combo.currentIndexChanged.connect(self._on_index_combo_changed)

        more_row.addWidget(self._index_combo, 1)
        ...
        v.addWidget(_lbl("行业筛选", _TEAL, 11, True))

        industry_row = QHBoxLayout()

        industry_row.setSpacing(6)

        self._industry_combo = QComboBox()

        self._industry_combo.setStyleSheet(_COMBO_SS)

        self._industry_combo.setFixedHeight(26)

        self._industry_combo.addItem("-- 选择行业 --", "")
        ...
        try:
            ...
            for ind in industries:
                ...
                self._industry_combo.addItem(ind, ind)
        ...
        self._industry_combo.currentIndexChanged.connect(self._on_industry_combo_changed)

        industry_row.addWidget(self._industry_combo, 1)

        # 更新行业缓存按钮

        ind_refresh_btn = QPushButton("🔄")

        ind_refresh_btn.setToolTip("从TuShare更新行业分类缓存")

        ind_refresh_btn.setFixedSize(26, 26)

        ind_refresh_btn.setStyleSheet(_small_btn_ss)

        ind_refresh_btn.clicked.connect(self._on_refresh_industry_cache)

        industry_row.addWidget(ind_refresh_btn)

        v.addLayout(industry_row)
    ...
    def _set_pool(self, pool):
        ...
        if isinstance(pool, str) and pool.startswith("IDX:"):

            # 指数成分股：从共享缓存加载

            index_code = pool[4:]
            _idx_names = {'000016': '上证50', '000300': '沪深300', '000905': '中证500',
                          '000852': '中证1000', '399006': '创业板指'}
            try:
                from vnpy.trader.index_constituents import SUPPORTED_INDICES
                for code, info in SUPPORTED_INDICES.items():
                    if code not in _idx_names:
                        _idx_names[code] = info.get('name', f'指数{code}')
            except Exception:
                pass
            self._current_pool_name = _idx_names.get(index_code, f'指数{index_code}')
    ...
    def _set_exchange_pool(self, exchange_key: str):

        _names = {'ALL': '全市场', 'SSE': '沪市', 'SZSE': '深市', 'BSE': '北交所'}
        self._current_pool_name = _names.get(exchange_key, exchange_key)
        """按交易所筛选股票（异步，不卡UI）"""

        self._pool_edit.setPlainText("加载中...")

        self._run_pool_loader(

            lambda: __import__("vnpy.trader.stock_pool", fromlist=["get_symbols_by_exchange"]).get_symbols_by_exchange(exchange_key),
    ...
    def _set_board_pool(self, board_name: str):
        ...
        self._run_pool_loader(
            ...
            lambda: __import__("vnpy.trader.stock_pool", fromlist=["get_symbols_by_board"]).get_symbols_by_board(board_name),
    ...
    def _run_pool_loader(self, loader_func, label: str):
        ...
        worker = _PoolWorker(loader_func)

        worker.finished.connect(lambda syms: self._on_pool_loaded(syms, label))

        worker.error.connect(lambda msg: self._on_pool_error(msg, label))
    ...
    def _on_pool_loaded(self, symbols: list, label: str):
    ...
    def _on_pool_error(self, msg: str, label: str):
        ...
        QMessageBox.warning(self, "筛选失败", f"{label}: {msg}")
    ...
    def _on_index_combo_changed(self, index: int):

        """从'更多指数'下拉框选择指数时，加载对应成分股"""
        ...
        # 从下拉框文本中提取指数名称
        text = self._index_combo.currentText().strip()
        if text:
            self._current_pool_name = text

        self._set_pool(data)

        # 选完后复位下拉框

        self._index_combo.blockSignals(True)

        self._index_combo.setCurrentIndex(0)

        self._index_combo.blockSignals(False)



    def _on_industry_combo_changed(self, index: int):

        """从行业下拉框选择行业时，加载对应股票（优化版）"""

        industry = self._industry_combo.currentData()
        ...
        try:
            ...
            if symbols:
                ...
                cache_file = Path.home() / ".vnpy" / "stock_pool" / "stock_industry.json"

                

                if not cache_file.exists():

                    self._status_lbl.setText("提示：行业筛选需先点击🔄更新缓存")

                else:

                    self._status_lbl.setText(f"'{industry}' 未找到股票")
            ...
            self._status_lbl.setText(f"行业筛选失败: {str(e)[:50]}")
        ...
        self._industry_combo.setCurrentIndex(0)

        self._industry_combo.blockSignals(False)



    def _reset_industry_combo(self):
    ...
    def _on_refresh_industry_cache(self):

        """从TuShare更新行业分类缓存"""

        try:

            from vnpy.trader.stock_pool import update_industry_cache
            ...
            self._status_lbl.setText("正在更新行业缓存...")

            

            # 更新缓存

            success, msg = update_industry_cache()
            ...
            QMessageBox.critical(self, "更新失败", f"更新行业缓存时出错：\n{str(e)}")
    ...
    def refresh_pattern_stats(self):
        ...
        """供形态统计 tab 的刷新按钮调用，重新运行研究"""

        self._on_run_research()
        ...
        class _ResearchWorker(QThread):
            ...
            def run(self):
                try:
                    ...
                    self._result_events_bars = events_bars
    ...
    def _on_research_finished(self, all_events: list):
        """子线程完成回调（在主线程执行）"""
        periods = self._research_periods
        symbols = self._research_symbols
        condition_expr = self._research_condition_expr

        events_bars = getattr(self._research_worker, "_result_events_bars", {})
        event_indices = getattr(self._research_worker, "_result_event_indices", {})
    ...
    def _generate_monitor_data(self, symbols: List[str], condition_expr: str):
        ...
        try:
            ...
            for _mi, sym_full in enumerate(symbols[:10]):
                if _mi % 2 == 0:
                    QApplication.processEvents()
